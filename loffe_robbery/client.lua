...
RegisterNetEvent('loffe_robbery:blip')
AddEventHandler('loffe_robbery:blip', function(Coords)
    CreateThread(function()
        local coordsped = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(robber)))
        local blip = AddBlipForCoord(coordsped)
        SetBlipSprite(blip, 161)
        SetBlipScale(blip, 0.8)
        SetBlipColour(blip, 1)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("Braquage Supérette")
                            ...
                            if canRob == true then
                                ...
                                while scared < 100 and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 do
                                    ...
                                    while sleep >= GetGameTimer() and not IsPedDeadOrDying(peds[i]) do
                                        Wait(0)
                                        DrawRect(0.5, 0.95, 0.2, 0.03, 75, 75, 75, 200)
                                        local draw = scared/500
                                        DrawRect(0.5, 0.95, draw, 0.03, 255, 10, 10, 200)
                                    end
                                    scared = scared + 1
                                end
