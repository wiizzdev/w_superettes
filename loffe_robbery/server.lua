...
AddEventHandler('loffe_robbery:pickUp', function(store)
    ...
    if Config.TypeMoney == 'sale' then
        xPlayer.addAccountMoney('black_money', randomAmount)
    elseif Config.TypeMoney == 'propre' then
        xPlayer.addMoney(randomAmount)
    end
    if Config.TypeMoney == 'sale' then
        TriggerClientEvent('esx:showNotification', source, Translation[Config.Locale]['cashrecieved'] .. ' ~r~' .. randomAmount .. ' ' .. Translation[Config.Locale]['currency'])
    elseif Config.TypeMoney == 'propre' then
...
RegisterServerEvent('loffe_robbery:alerte')
AddEventHandler('loffe_robbery:alerte', function(CodeCoords, CodeType)
    local xPlayers = ESX.GetPlayers()
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
        if thePlayer.job.name == 'police' then
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], "Police ", "Braquage de Banque", "La Pacific Bank se fait braquer !\nRendez-vous sur place le plus vite possible !", 'CHAR_DIAL_A_SUB', 0)
            TriggerClientEvent('braco:blip', xPlayers[i], CodeCoords)
        end
    end
end)
...
AddEventHandler('loffe_robbery:rob', function(store)
    ...
    for i = 1, #xPlayers do
        ...
        if xPlayer.job.name == 'police' then
            ...
            TriggerClientEvent('loffe_robbery:blip', xPlayer.source, store, src)
        end
    end
...
Citizen.CreateThread(function()
    while true do
        for i = 1, #deadPeds do TriggerClientEvent('loffe_robbery:pedDead', -1, i) end
        Citizen.Wait(500)
    end
end)